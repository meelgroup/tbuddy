...
public:
  Term (BDD r) { id = next_id++; root = bdd_addref(r); is_active = true; }
  Term (Clause *clp) { id = next_id++; root = bdd_addref(build_from_clause(clp)); is_active = true; }
...
public:
  ...
  Term *conjunct(Term *tp1, Term *tp2) {
    BDD nroot = bdd_and(tp1->get_root(), tp2->get_root());
    add(new Term(nroot));
    tp1->deactivate();
    tp2->deactivate();
    and_count++;
